...
class PCA9685:
  ...
  def setRotationAngleGradually(self, channel, angle, prevAngle): 
    if(angle >= 0 and angle <= 180):
        prevPulse = prevAngle * (2000 / 180) + 501
        pulse = angle * (2000 / 180) + 501
        
        d = 5
        if ((prevPulse - pulse) > 0 ):
          d = -5
        
        for d_pulse in range(int(prevPulse), int(pulse), d):
          time.sleep(0.03)
          if (d_pulse > 500 and d_pulse < 2500):
            self.setServoPulse(channel, d_pulse)
